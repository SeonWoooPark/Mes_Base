...
// Application Layer
import { GetProductListUseCase, ProductPresenter } from '../application/usecases/product/GetProductListUseCase';
  ...
  static initialize(
    ...
    if (useMockData) {
      ...
      diContainer.register(PRODUCT_TOKENS.ProductRepository, {
        useFactory: (c) => new HttpProductRepository(c.resolve(PRODUCT_TOKENS.ApiClient))
      });
      diContainer.register(PRODUCT_TOKENS.ProductHistoryRepository, {
        useFactory: (c) => new HttpProductHistoryRepository(c.resolve(PRODUCT_TOKENS.ApiClient))
      });
    }
    ...
    diContainer.register(PRODUCT_TOKENS.ProductCodeGenerator, {
      useFactory: (c) => new DefaultProductCodeGenerator(
        c.resolve(PRODUCT_TOKENS.ProductRepository)
      )
    });
    
    // ProductUsageChecker (현재는 Mock만 구현)
    diContainer.registerSingleton(PRODUCT_TOKENS.ProductUsageChecker, MockProductUsageChecker);
    
    // ProductPresenter
    diContainer.registerSingleton(PRODUCT_TOKENS.ProductPresenter, ProductPresenterImpl);
    ...
    diContainer.register(PRODUCT_TOKENS.GetProductHistoryUseCase, {
      useFactory: (c) => new GetProductHistoryUseCase(
        c.resolve(PRODUCT_TOKENS.ProductHistoryRepository)
      )
    });
